...
- **Commit Flow:** A global `pointerdown` listener on the `document` (using the capture phase) intercepts clicks outside the editor or toolbar and triggers `commitTextLayer()`. This hides the overlay, stores `htmlContent`/`textContent`/`lineHeight` on the layer object, and re-renders the text to the canvas. Empty text layers are automatically deleted. Switching to another tool also commits the active text layer.
...
- **Pointer/Pen Interactions:** Ensure that any new interactive or draggable custom elements (e.g., color map, sliders, modal dialogs) have `touch-action: none;` applied to prevent native browser touch gestures (such as panning or pull-to-refresh) from interfering with drawing and dragging.
